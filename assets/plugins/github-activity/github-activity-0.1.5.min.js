...
function millisecondsToStr(a) {
    ...
    var c = Math.floor(a / 1e3), d = Math.floor(c / 31536e3);
...
function md51(a) {
    txt = "";
    var b, c = a.length, d = [1732584193, -271733879, -1732584194, 271733878];
    ...
    if (e[b >> 2] |= 128 << (b % 4 << 3), b > 55) for (md5cycle(d, e), b = 0; 16 > b; b++) e[b] = 0;
...
var GitHubActivity = function () {
    ...
    var a = {}, b = {
        renderLink: function (a, b, c) {
            ...
            var c = a.payload;
            if (a.repoLink = b.renderGitHubLink(a.repo.name), a.userGravatar = Mustache.render('<div class="gha-gravatar-user"><img src="{{url}}" class="gha-gravatar-small"></div>', {url: a.actor.avatar_url}), c.ref && (a.branch = "refs/heads/" === c.ref.substring(0, 11) ? c.ref.substring(11) : c.ref, a.branchLink = b.renderGitHubLink(a.repo.name + "/tree/" + a.branch, a.branch) + " at "), c.commits) {
                var d = c.before + "..." + c.head, e = c.commits.length;
                2 === e ? a.commitsMessage = Mustache.render('<a href="https://github.com/{{repo}}/compare/{{shaDiff}}">View comparison for these 2 commits &raquo;</a>', {
                    ...
                    return d.message.length > 66 && (d.message = d.message.substring(0, 66) + "..."), 2 > e ? (d.shaLink = b.renderGitHubLink(a.repo.name + "/commit/" + d.sha, d.sha.substring(0, 6), "gha-sha"), void (d.committerGravatar = Mustache.render('<img class="gha-gravatar-commit" src="assets/images/github/octocat.png" width="16" />', {hash: md5(d.author.email)}))) : (c.commits.splice(2, c.size), !1)
                })
            }
            if (c.issue) {
            ...
            if (c.comment && c.comment.pull_request_url) {
                ...
                a.pullRequestLink = b.renderLink(c.comment.html_url, f)
            }
            ...
            var j, h = Mustache.render(templates[a.type], a), k = millisecondsToStr(new Date - new Date(a.created_at));
            j = "CreateEvent" == a.type && ["repository", "branch", "tag"].indexOf(c.ref_type) >= 0 ? icons[a.type + "_" + c.ref_type] : icons[a.type];
            var l = {message: h, icon: j, timeString: k, userLink: b.renderGitHubLink(a.actor.login)};
            ...
            return a.name ? a.userNameLink = b.renderLink(a.html_url, a.name) : a.withoutName = " without-name", a.userLink = b.renderLink(a.html_url, a.login), a.gravatarLink = '<img src="assets/images/github/icon.png">', Mustache.render(templates.UserHeader, a)
            ...
            if (d instanceof HTMLCollection) for (var e = 0; e < d.length; e++) b.renderStream(c, d[e]); else b.renderStream(c, d)
    ...
    return a.feed = function (a) {
        ...
        var c, d, e = a.selector, f = "https://api.github.com/users/" + a.username, g = f + "/events";
        ...
        if (a.eventsUrl && (g = a.eventsUrl), "object" == typeof a.templates) for (var i in templates) "string" == typeof a.templates[i] && (templates[i] = a.templates[i]);
        b.getOutputFromRequest(f, function (f, g) {
            c = f ? Mustache.render(templates.UserNotFound, {username: a.username}) : b.getHeaderHTML(g), b.renderIfReady(e, c, d)
        }), b.getOutputFromRequest(g, function (f, g) {
            if (f) d = Mustache.render(templates.EventsNotFound, {username: a.username}); else {
...
var templates = {
        ...
        SingleLineActivity: '<div class="gha-activity gha-small">                         <div class="gha-activity-icon"><span class="octicon octicon-{{icon}}"></span></div>                         <div class="gha-message"><div class="gha-time">{{{timeString}}}</div>{{{userLink}}} {{{message}}}</div>                         <div class="gha-clear"></div>                       </div>',
        ...
        PushEvent: 'pushed to {{{branchLink}}}{{{repoLink}}}<br>                <ul class="gha-commits">{{#payload.commits}}<li><small>{{{committerGravatar}}} {{{shaLink}}} {{message}}</small></li>{{/payload.commits}}</ul>                <small class="gha-message-commits">{{{commitsMessage}}}</small>',
